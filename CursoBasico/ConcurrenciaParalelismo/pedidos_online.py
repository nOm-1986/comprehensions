...
async def process_order(order_id, items):
    ...
    if not all(inventory_results):
        print(f'Orden {order_id} was canceled: This product is not available."')
    ...
    if payment_result:
        print(f'Orden {order_id} completed. Total: {total}')
    else:
        print(f'It was an error processing payment {order_id}')
